...
app.patch('/employees/', (req, res, next) => {
    const { last_name, first_name, title, address, country_code, employee_id } = req.body;
    db.run(`UPDATE employees SET last_name = ?, first_name = ?, title = ?, address = ?, country_code = ? WHERE employee_id = ?`, [last_name, first_name, title, address, country_code, employee_id], function (err, result) {
        if (err) {
            return res.status(400).json({ 'error': err.message });
        }
