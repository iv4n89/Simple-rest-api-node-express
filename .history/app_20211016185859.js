...
app.post('/employees/', (req, res, next) => {
    ...
    db.run(`INSERT INTO employees (last_name, fist_name, title, address, country_code) VALUES (?,?,?,?,?)`, [last_name, first_name, title, address, country_code], function (err, result) {
        if (err) {
            return res.status(400).json({ error: err.message });
        }
