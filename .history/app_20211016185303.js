...
app.post('/employees/', (req, res, next) => {
    const { last_name, fist_name, title, address, country_code } = req.body;
    db.run(`INSERT INTO employees (last_name, fist_name, title, address, country_code) VALUES (?,?,?,?,?)`, [last_name, fist_name, title, address, country_code], function (err, result) {
        if (err) {
            return res.status(400).json({ error: err.message });
        }
